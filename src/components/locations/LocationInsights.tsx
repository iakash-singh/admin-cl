...
export default function LocationInsights() {
  ...
  const totalVendors = mockLocations.reduce(
    (sum, loc) => sum + loc.vendorCount,
    0
  );
  const averageOrderValue = (() => {
    const locationsWithOrders = mockLocations.filter((loc) => loc.orderCount > 0);
    if (locationsWithOrders.length === 0) return 0;
    const total = locationsWithOrders.reduce(
      (sum, loc) => sum + loc.revenue / loc.orderCount,
      0
    );
  ...
  const locationColumns = [
    ...
    },
    {
      ...
      render: (_value: number, row: Location) => {
        const density =
          row.vendorCount > 0 ? row.userCount / row.vendorCount : null;
        return (
          <div className="flex items-center">
      ...
      >
              ...
              {[...mockLocations]
                ...
                .map((location, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900">
                        {location.opportunity.toFixed(1)}
                      </p>
        ...
        {selectedLocation && (
          <motion.div
            ...
            onClick={(e) =>
              e.target === e.currentTarget && setSelectedLocation(null)
            }
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full border border-gray-100 overflow-hidden"
            >
